...
| Bloc 4 | Dossier écrit 20p max | **20–24/07/2026 — ⚠️ échéance dépassée, dépôt à confirmer** | 🟢 **dossier v5 généré le 14/08 (retours prof intégrés)** : annexes de preuves ajoutées — captures GitHub des fichiers de dépendances (composer.json, package.json, dependabot.yml), e-mails d'alerte/rétablissement réels + journaux de sonde et canal Monolog (exercice d'incident du 14/08 : base arrêtée puis relancée), ticket GitHub BUG-011 + vue registre (**17 issues créées #12→#28**, 16 closes + OBS-01 ouverte), CHANGELOG sur master — **18 pages/20**, OOXML PASSED. Au passage : le canal Monolog `incident` est désormais réellement alimenté par /api/health (commit `275503fc`). v4/v3 conservées (`Dossier_Bloc4_Albion_Helper_v3.docx`, 13 pages/20) — corrige les incohérences de la v2 (figée depuis le 15/07) : 173 tests partout (était 131), journal des versions à jour (ligne v3.2.0 ajoutée), §3.4 UptimeRobot décrite comme activée (au lieu de « à activer »), recommandation §7 « remettre la production » retirée du tableau (réalisée) + phrase de crédibilité ajoutée ; reste : capture d'écran réelle UptimeRobot à insérer en §3.4 (je n'ai pas accès au dashboard), relire dans Word, **confirmer si le dépôt DigiformaCertif du 20–24/07 a bien eu lieu** sinon déposer en urgence |
...
- [x] ~~Capture UptimeRobot à insérer dans le dossier §3.4~~ **fait le 14/08** : Figure 2 de la v4 (cf. revue du 14/08 plus bas)
...
- [x] ⚠️ ~~Constat SSH du 14/08 : sonde interne morte sur le VPS~~ **réparée le 14/08 en fin de journée**. Le VPS avait été altéré depuis le 20/07 (probable montée de version OS : PHP 8.2→8.4, **composer/node/npm disparus**, crontab `albion` vidée, clé SSH de la CI refusée). Réparations : ① script de sonde corrigé (v3.3.1 : journal avec repli `$HOME`, garde `MAIL-SKIP` car `mail` absent du VPS) ; ② `deploy.sh` exporte `PATH=$HOME/bin` (commit `c7e8d185`) ; ③ **composer 2.10 + node v22.23 réinstallés sans root dans `~albion/bin`** ; ④ `deploy.sh` rejoué en SSH → déploiement complet OK (migrations, build, gzip, « Sonde installée ») ; ⑤ crontab `*/5` vérifiée présente, `/api/health` → 200 OK. Nota : au premier passage la crontab est restée vide (deploy.sh s'auto-remplace en plein vol via `git reset` — le vieux code continue de s'exécuter) → toujours relancer une 2ᵉ fois après un gros changement de deploy.sh, ou réécrire la ligne à la main
...
- [x] 📷 ~~Capture UptimeRobot §3.4 manquante~~ **insérée le 14/08 dans la v4** (Figure 2, prise en direct sur dashboard.uptimerobot.com : statut Up depuis 1j22h, 100 % sur 24 h, 99,95 % sur 7 j, latence moyenne 635 ms — cibles du §3.1 tenues ; l'ancienne Figure 2 du pipeline devient Figure 3) — validation OOXML PASSED, toujours 13 pages
...
- [x] 🔁 **CD GitHub Actions réparée le 14/08 au soir** : les secrets vivent dans l'**environnement GitHub « production »** (pas au niveau repo — c'est pour ça que `gh secret list` semblait vide) ; nouvelle paire RSA-4096 dédiée générée (`~/.ssh/albion_ci_deploy` sur le Mac), clé publique posée sur le VPS, `SSH_PRIVATE_KEY` + `SSH_USER` mis à jour dans l'environnement production → **job deploy vert en 44 s** (run 31811147147)
- [x] 🐞 **Cause racine de la disparition de la sonde identifiée et corrigée** (commit `c3e98030`) : sous `set -e`, quand la sonde était la seule entrée de la crontab, `grep -v` sortait en code 1 et tuait le sous-shell avant le `echo` → `crontab -` recevait un flux vide et **effaçait la crontab à chaque déploiement**. Ça marchait en juillet uniquement grâce à la vieille sonde parasite encore présente ; son nettoyage du 19/07 a armé la bombe pour le déploiement du 20/07. Garde `|| true` ajoutée + CHANGELOG v3.3.1 complété. **Validé par un déploiement CD à vide : run vert + crontab intacte après passage** (attention résiduelle : quand `deploy.sh` lui-même est modifié, le déploiement en cours exécute encore l'ancienne version — l'effet du correctif ne se voit qu'au déploiement suivant). Belle histoire d'incident à raconter au jury (C4.2.1/C4.2.2 vécu en vrai)
