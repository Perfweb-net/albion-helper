...
const KillEventCard = ({ event, perspective = 'kill' }) => {
    ...
    return (
        <Card variant="outlined" sx={{ borderLeft: `4px solid ${win ? '#4ade80' : '#f87171'}` }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, p: 1.5, flexWrap: 'wrap' }}>
                <Chip
                    size="small"
                    label={win ? t('pvp.kill') : t('pvp.death')}
                    sx={{ fontWeight: 700, bgcolor: win ? 'rgba(74,222,128,0.15)' : 'rgba(248,113,113,0.15)', color: win ? '#4ade80' : '#f87171' }}
                />
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flex: 1, minWidth: 220 }}>
                    <PlayerLine player={event.killer} color="#4ade80" t={t} />
                    <Typography variant="caption" color="text.secondary">{t('pvp.killed')}</Typography>
                    <PlayerLine player={event.victim} color="#f87171" t={t} />
                </Box>
            ...
            <Collapse in={open} unmountOnExit>
                ...
                <Box sx={{ p: 2, display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                    <Box>
                        <Typography variant="caption" sx={{ color: '#4ade80', fontWeight: 700 }}>{t('pvp.killer_equipment')}</Typography>
                        <Box sx={{ mt: 0.5 }}><EquipmentLoadout equipment={event.killer?.equipment} /></Box>
                    </Box>
                    <Box>
                        <Typography variant="caption" sx={{ color: '#f87171', fontWeight: 700 }}>{t('pvp.victim_equipment')}</Typography>
